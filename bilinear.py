def image_warp(img, depth, padding_mode='zeros'):
    ...
    b, _, h, w = depth.size() 
    i_range = torch.autograd.Variable(torch.linspace(-1.0, 1.0).view(1, h, 1).expand(1,h,w),requires_grad = False) # [1, H, W]  copy 0-height for w times : y coord
    j_range = torch.autograd.Variable(torch.linspace(-1.0, 1.0).view(1, 1, w).expand(1,h,w),requires_grad = False) # [1, H, W]  copy 0-width for h times  : x coord
